...
void cv_color_tracking(const Mat& input_img, ros::Publisher &controlPub)
{
    ...
    cv::inRange(imgHSV, cv::Scalar(105, 150, 50), cv::Scalar(125, 255, 255), mask1); // red
    cv::Mat1b mask = mask1;
    ...
    for (size_t i = 0; i < num_contours; i++) {
        ...
        Rect b = boundRect[i];
        int posX = b.x + b.width/2;
        int posY = b.y + b.height/2;
        
        double width = input_img.size().width;
        if (area == biggest)
        {
            if (posX < width/3)
                pos = "left";
            else if (posX < 2 * width/3)
                pos = "center";
            else
                pos = "right";
        }
        ...
        int baseline;
        Size text = getTextSize("x", FONT_HERSHEY_SIMPLEX, 1.0, 1, &baseline);
        posX -= (int) text.width/2;
        posY += (int) text.height/2;
        if (posX >= 0 && posY >= 0)
        {
            putText(input_img, "x", Point(posX, posY), FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255));
            posY -= text.height;
            //putText(input_img, pos, Point(posX, posY), FONT_HERSHEY_SIMPLEX, .4, (0, 0, 0));
